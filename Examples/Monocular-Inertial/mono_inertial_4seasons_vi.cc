...
void LoadImages4SeasonsVI(const string &strSequence, vector<string> &vstrImageFilenames,
...
int main(int argc, char **argv)
{
    if(argc != 4)
    {
        cerr << endl << "Usage: ./mono_inertial_4seasons_vi path_to_vocabulary path_to_settings path_to_sequence_record" << endl;
        return 1;
    }
    ...
    LoadImages4SeasonsVI(string(argv[3]), vstrImageFilenames, vTimestampsCam);
    int nImages = vstrImageFilenames.size();

    vector<cv::Point3f> vAcc, vGyro;
    vector<double> vTimestampsImu;
    LoadIMU(string(argv[3]) + "/imu.txt", vTimestampsImu, vAcc, vGyro);
...
void LoadImages4SeasonsVI(const string &strPathToSequence, vector<string> &vstrImageFilenames, vector<double> &vTimestamps)
{
    ifstream fTimes;
    string strPathTimeFile = strPathToSequence + "/times.txt";
    string strPrefixLeft = strPathToSequence + "/distorted_images/cam0/";
    ...
    while(!fTimes.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            string frameID;
            ss >> frameID;
            vstrImageFilenames.push_back(strPrefixLeft + frameID + ".png");
            double t;
            ss >> t;
