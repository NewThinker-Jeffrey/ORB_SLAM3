...
void LoadImagesEuRoCMav(const string &strPathLeft, const string &strPathRight, const string &strPathTimes,
...
int main(int argc, char **argv)
{
    ...
    if(argc < 7) 
    {
        cerr << endl << "Usage: ./stereo_inertial_euroc_mav path_to_vocabulary path_to_settings path_to_image_folder_1 path_to_image_folder_2 path_to_times_file path_to_imu_data (trajectory_file_name)" << endl;
        return 1;
    }
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        LoadImagesEuRoCMav(string(argv[4*(seq+1)-1]), string(argv[4*(seq+1)]), string(argv[4*(seq+1)+1]), vstrImageLeftFilenames[seq], vstrImageRightFilenames[seq], vTimestampsCam[seq]);
                ...
                vector<string> &vstrImageLeft, vector<string> &vstrImageRight, vector<double> &vTimeStamps)
{
    ...
    while(!fTimes.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            if (suffix.empty()) {
                std::string try_file_name = strPathLeft + "/" + item + ".png";
                if (checkFile(try_file_name)) {
                    suffix = ".png";
                } else {
                    suffix = ".jpg";
                }
            ...
            vstrImageLeft.push_back(strPathLeft + "/" + item + suffix);
            vstrImageRight.push_back(strPathRight + "/" + item + suffix);
