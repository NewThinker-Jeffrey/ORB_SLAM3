/**
* 
* This file is added by robotera to test realsense-D455.
*
*/
...
int main(int argc, char **argv) {
    ...
    int gyro_cb_cnt = 0;
    int acc_cb_cnt = 0;
    int unknown_cb_cnt = 0;
    auto imu_callback = [&](const rs2::frame& frame)
    {
        cout << fixed << setprecision(6);
        if ((++total_cb_cnt) % 100 == 0) {
            // cout << "RS_DEBUG: total callback count = " << total_cb_cnt << std::endl;
        }
        std::unique_lock<std::mutex> lock(imu_mutex);
        ...
        else if (rs2::motion_frame m_frame = frame.as<rs2::motion_frame>())
        {
            ...
            // if ((++unknown_cb_cnt) % 100 == 0) {
                cout << "RS_DEBUG: UNKNOWN callback count = " << unknown_cb_cnt << std::endl;
            // }
        }
    };
    ...
    {
        if ((++loop_cnt) % 30 == 0) {
            cout << "SLAM_DEBUG: main loop count = " << loop_cnt << std::endl;
        }
...

        {
            ...
            if (!b_continue_session) {
                cout << "Terminated by user! Now shutdown SLAM ...\n";
                SLAM.Shutdown();
                continue;
            }
